...
function treecolor(){
    ...
    $("body").on("click",".mis-shrink-tree",tree_slide);
...
function bdhd_edit(){
    $("body").on("click",".mis-bdhd-edit .edit",function(){
       $('.mis-bd-tablist').find(".del").css({"display":"block"});
       $('.mis-bd-tablist li').addClass('deltbg')
    })
    $("body").on("click",".mis-bd-tablist .del",function(){
        var hasDel = confirm('确定删除吗？');
        //确定执行函数
        if(hasDel){
            $(this).parents(".deltbg").remove();
        }
